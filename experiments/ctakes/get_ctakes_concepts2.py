...
from PyStream.src.stream import Stream
...
def concept_extractor(doc: Document):
    concepts = to_records(doc.concepts)
    for concept in concepts:
        concept["file"] = os.path.basename(doc.path)
    return concepts
...
if __name__ == "__main__":
    ...
    pipeline = Stream(files).through(Document).through(concept_extractor).through(csvfile.writerows)

    # Loop over directory
    for _ in tqdm(pipeline, desc="Parsing docs", unit=" doc"):
        pass
